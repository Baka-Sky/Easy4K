            ...
            Content = new ScrollViewer
            {
                ...
                Content = new TextBlock
                {
                    ...
                    Text =
                        ...
                        "多算一份插值丢掉不用，因此块边界与整段处理完全一致。实测同一素材：超分帧、补帧帧、去重判决与回填展开结果" +
                        "逐帧 MD5 全部相同（140 帧 360p 与 300 帧 360p 两组，开/关帧去重各一组），成品时长、帧率、音频对齐与普通模式一致。\n\n" +
                        "【单卡的真实收益】\n" +
                        "超分与补帧都是 GPU 密集阶段，只有一块显卡时同时跑会互相抢算力：各自速率减半、总吞吐基本不变。" +
                        "实测（RX 580，300 帧 360p）：串行 74.7s，涡轮 72.7s（线程 1:2:2）到 77.7s（线程 1:4:4）—— 也就是 ±5% 以内；" +
                        "68 帧 1080p 为串行 129.2s 对涡轮 128.1s。开帧去重时省下的是判决那一整段 CPU 时间（判得越慢、素材越长省得越多），" +
                        "但会被按块复制的额外读写抵消一部分，实测 140 帧小片段为 34.4s 对 34.4s 持平。\n" +
                        "结论：单卡上不必期待提速，它的意义在于结果与普通模式完全一致的前提下把 CPU 侧的活压进 GPU 空档；" +
                        "素材很长且开了完美模式去重时收益才明显。开启后建议自己拿一段素材对比耗时，按实际结果决定留不留。\n\n" +
                        "【多卡分流】\n" +
