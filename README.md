...
- **拆帧** — 使用 FFmpeg 将视频按原始质量逐帧导出 PNG（可选 GPU 硬解加速）
- **AI 超分** — 集成 Real-ESRGAN-ncnn-vulkan，支持 x2 / x3 / x4 倍率与多个模型
- **AI 补帧** — 支持两种引擎：NCNN（rife-ncnn-vulkan，40+ 模型）与 Offical（PyTorch 官方实现，10+ 模型），2 倍帧率提升
- **合并视频** — FFmpeg (libx265 / CRF 18) 将处理后的帧合成新视频（可选 GPU 硬件编码器，失败自动回退 CPU）
- **音频嵌入** — 自动提取原视频音轨并嵌入新视频（PCM 24bit / 96kHz）
- **SDR→HDR** — NVIDIA RTX 显卡可开启（NVEncC 驱动），非 RTX 自动禁用
- **CPU 处理模式** — 勾选后全部模型使用 CPU 推理（超分不支持 CPU 时自动升级 GPU 处理），适合 GPU 不可用/不稳定时
- **安全帧率** — 遇到设备丢失/显存/内存溢出时强制停止，保护显卡
- **降低画质** — `-u` UHD / Offical FP16，降低显存/内存占用
- **实时进度** — 每阶段独立进度条 + 逐帧计数 + 实时日志
- **CPU/GPU 监控** — 实时显示占用率（环形进度指示器）
- **预览图** — 处理过程中实时预览最新帧（新帧加载完成前保持旧帧，不闪烁）
- **断点续传** — 已完成的阶段自动跳过，中断后重开可复用中间结果
- **临时文件清理** — 一键清理中间产物，带成功/失败结果提示；临时目录含旧文件时启动前弹窗引导（重新选择/清理/取消）
- **关闭保护** — 处理中关闭窗口会弹窗确认，选择"关闭"将强制终止所有子进程
- **主题切换** — 浅色 / 深色 / 跟随系统，自动保存
- **设置持久化** — 模型、线程数、各开关、临时/输出目录等自动保存到 `appsettings.json`
...
4. 选择超分模型（按倍率自动过滤）与补帧模型；补帧引擎可在 **NCNN / Offical** 之间切换
5. （可选）在**高级性能选项**中调整：
   - **开启安全帧率** — 遇到设备丢失/显存/内存溢出时强制停止（默认开启）
   - **降低部分画质** — `-u` UHD / Offical FP16，降低显存/内存占用
   - **使 FFmpeg 尝试使用 GPU 加速** — 拆帧 `-hwaccel` / 合并帧 GPU 编码器优先，失败自动回退 CPU
   - **使用 CPU 处理所有模型** — 全部模型 CPU 推理（速度慢，仅 GPU 不可用时建议；开启后自动禁用以上三项并取消勾选）
6. 点击 **"开始处理"**（快捷键 `Ctrl+S`）
   - 处理中实时查看：阶段进度、逐帧进度、预览图、命令日志、CPU/GPU 占用
   - 临时目录有旧文件时会弹窗引导清理/更换；CPU 模式会弹最终确认
7. 输出文件位于 `Output/` 目录
...
- **参数区** — 超分与补帧的模型、倍率选择（补帧引擎 NCNN / Offical 可切换）
- **高级性能选项** — 安全帧率、降低画质、FFmpeg GPU 加速、CPU 处理模式（CPU 开启时前三项自动取消并禁用）
...
**NCNN（rife-ncnn-vulkan，默认）** — 默认推荐 `rife-v4.6`（兼容性最好）。内置 40+ 模型：
`rife-v2` / `v2.3` / `v2.4` / `v3.x` / `v4.x`（含 `-lite` / `-HD` / `-UHD` / `-anime` / `-large` / `-heavy` 变体）。

**Offical（PyTorch 官方实现）** — 使用 Python 运行时加载官方 pkl 模型，内置 10+ 模型：
`official_2.3` / `4.6` / `4.8` / `4.9` / `4.15` / `4.18` / `4.22` / `4.26` / `4.26_heavy` / `rpr_v7_2.3`。

- 新版模型（如 v4.25 / v4.26）画质更好，但需要 8GB+ 显存
- 显存不足时软件会给出警告；RIFE 模型不被命令行版支持时（如 v4.25/v4.26 报 `layer MemoryData not exists`）会自动回退 `rife-v4.6`，并在进行中页面标注"**因硬件原因回退至 rife-v4.6**"
- 低显存显卡运行大模型失败时自动降线程重试
- 无 CUDA 时 Offical 引擎自动降级为 CPU 推理（开启 CPU 模式则强制 CPU）
...
补帧   rife-ncnn-vulkan -n {总帧数} -u       → Temp/output_frames   （NCNN）
补帧   python run.py -m {模型目录} -mult 2    → Temp/output_frames   （Offical）
...
**Q8: 开启"使用 CPU 处理"后超分还在用 GPU？**
→ 捆绑的 realesrgan-ncnn 构建无 CPU 后端（`-g -1` 无效），超分会自动**升级为 GPU 处理**（日志显示"超分中(升级GPU)"）；补帧（NCNN `-g -1` / Offical `-cpu`）会正常走 CPU。

**Q9: 提示"临时目录有旧文件"怎么办？**
→ 弹窗提供"重新选择临时目录 / 清理缓存 / 取消"。点"开始处理"每次都弹（未处理前不会启动），连续 5 次不处理会转为强烈提醒；清理或更换到干净目录后自动放行。

**Q10: 为什么启动按钮灰色？**
→ 常见于未选择有效视频、输出/临时目录为空等。选择视频并设置目录后即可点击；临时目录有旧文件时按钮不再置灰，点击后弹窗引导处理。
